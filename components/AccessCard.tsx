...
import { Check, Lock, Sparkles, Globe, Phone, ArrowLeft, ArrowRight, ExternalLink } from 'lucide-react';
...
export const AccessCard: React.FC = () => {
  ...
  const [validation, setValidation] = useState<ValidationState>({ isValid: false, message: '', type: 'idle' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
   ...
   if (!inputKey.trim()) {
  setValidation({ isValid: false, message: 'Vui lòng nhập mã kích hoạt.', type: 'error' });
  return;
}

if (!isFullAccess && !selectedCourseId) {
  setValidation({ isValid: false, message: 'Vui lòng chọn môn học.', type: 'error' });
  return;
}

setValidation({ isValid: true, message: 'Kích hoạt thành công! Đang chuyển hướng...', type: 'success' });
  };

  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(CONTACT_INFO.website)}`;

  const RobotLogo = ({ size = "normal" }: { size?: "normal" | "medium" | "small" | "tiny" }) => {
    const containerClass = size === "normal" ? "w-20 h-20" : size === "medium" ? "w-16 h-16" : size === "small" ? "w-12 h-12" : "w-10 h-10";
    const scaleClass = size === "normal" ? "scale-[0.75]" : size === "medium" ? "scale-[0.6]" : size === "small" ? "scale-[0.5]" : "scale-[0.4]";
  ...
  <div className={`relative group cursor-pointer transition-transform hover:scale-105 duration-300`}>
    <div className={`${containerClass} bg-[#2d8eff] ${borderRadius} shadow-xl shadow-blue-500/30 flex items-center justify-center relative overflow-hidden`}>
       ...
       <div className={`relative z-10 mt-1 transform ${scaleClass} origin-center`}>
  ...
  return (
    ...
    <div className="absolute inset-0 w-full h-full bg-white rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100 backface-hidden flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-between p-6 bg-gradient-to-b from-blue-50/80 to-white relative">
        ...
        <div className="flex flex-col items-center mt-2">
          ...
          <h1 className="mt-3 text-xl font-black text-gray-800 tracking-tight uppercase">Tin Học Cơ Bản</h1>
          <p className="text-xs text-gray-500 font-medium">Học thật - Làm thật - Kết quả thật</p>
        </div>

        {/* QR Code */}
        <div className="p-2 bg-white rounded-xl shadow-md border border-gray-100 relative group">
           <img src={qrUrl} alt="Website QR" className="w-28 h-28 rounded-lg" />
        ...
        <div className="w-full space-y-2">
           <a href={CONTACT_INFO.website} target="_blank" rel="noreferrer" className="flex items-center p-2.5 bg-gray-50 rounded-lg hover:bg-blue-50 hover:shadow-sm transition-all border border-gray-100 group cursor-pointer">
              ...
              <div>
                <div className="text-[9px] uppercase font-bold text-gray-400">Website</div>
                <div className="font-bold text-sm text-gray-800">{CONTACT_INFO.website.replace('https://', '')}</div>
              </div>
              <ExternalLink size={14} className="ml-auto text-gray-300 group-hover:text-blue-400" />
           </a>
           <div className="grid grid-cols-2 gap-2">
              <a href={`https://${CONTACT_INFO.tiktok}`} target="_blank" rel="noreferrer" className="flex flex-col items-center p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors border border-gray-100">
                <div className="mb-0.5">
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-black"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/></svg>
                </div>
                <div className="font-bold text-[10px] text-gray-700">@tinhoccobann</div>
              </a>
              <div className="flex flex-col items-center p-2 bg-blue-50 rounded-lg border border-blue-100">
                <div className="flex items-center gap-1 mb-0.5">
                   <Phone size={14} className="text-blue-600" />
                   <span className="bg-blue-600 text-white text-[7px] font-bold px-1 rounded">ZALO</span>
                </div>
                <div className="font-bold text-[10px] text-blue-700">{CONTACT_INFO.phone}</div>
              </div>
           </div>
        </div>
    ...
    <div className="absolute inset-0 w-full h-full bg-white rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100 backface-hidden rotate-y-180 flex flex-col p-4">
      ...
      <div className="mt-1 flex flex-col items-center justify-center shrink-0">
         ...
         <h2 className="mt-2 font-bold text-gray-900 text-lg tracking-tight">Tin Học Cơ Bản</h2>
      ...
      <div className="flex-1 flex flex-col justify-center gap-2 py-1 min-h-0 overflow-hidden">
         ...
         <div 
           ...
           className={`
             ...
             ${isFullAccess ? 'ring-1 ring-offset-0 border-violet-500' : 'hover:shadow-md hover:border-violet-300'}
           `}
         >
           {/* Checkbox */}
           <div className={`
             ...
             ${isFullAccess ? 'border-transparent' : 'group-hover:border-gray-400'}
           `}>
             {isFullAccess && <div className="w-2 h-2 rounded-full bg-violet-600"></div>}
           </div>
           ...
           <div className="flex-1 min-w-0">
               <h3 className="font-bold text-xs text-violet-700">Combo Tất Cả Khóa Học</h3>
           </div>
         </div>
         ...
         <div className="bg-gray-50/80 p-3 rounded-xl border border-gray-100">
            <div className="flex items-center justify-center mb-2">
               <label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1.5">
                  <Lock size={10} className="text-gray-400" /> Mã Kích Hoạt
               </label>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-2">
               ...
               {validation.type !== 'idle' && (
                  <div className={`text-[9px] text-center font-bold -mt-1 ${validation.type === 'error' ? 'text-red-500' : 'text-green-500'}`}>
                    {validation.message}
                  </div>
               )}
